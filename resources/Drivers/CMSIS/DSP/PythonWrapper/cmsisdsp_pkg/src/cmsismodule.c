...
#define MLTYPE(name, thenewfunc, deallocfunc, initfunc, methods)                                               \
    static PyTypeObject ml_##name##Type = {PyVarObject_HEAD_INIT(NULL, 0).tp_name = MODNAME ".##name",         \
...
#define INITARRAYFIELD(FIELD, FORMAT, SRCFORMAT, DSTFORMAT)                                              \
    ...
    {                                                                                                    \
        ...
        PyArrayObject *FIELD##c = (PyArrayObject *) PyArray_FromAny(                                     \
            FIELD, desct, 1, 0, NPY_ARRAY_C_CONTIGUOUS | NPY_ARRAY_ALIGNED | NPY_ARRAY_FORCECAST, NULL); \
        if (FIELD##c)                                                                                    \
        {                                                                                                \
...
#define GETCARRAY(PYVAR, CVAR, FORMAT, SRCFORMAT, DSTFORMAT)                                             \
    ...
    {                                                                                                    \
        ...
        PyArrayObject *PYVAR##c = (PyArrayObject *) PyArray_FromAny(                                     \
            PYVAR, desct, 1, 0, NPY_ARRAY_C_CONTIGUOUS | NPY_ARRAY_ALIGNED | NPY_ARRAY_FORCECAST, NULL); \
        if (PYVAR##c)                                                                                    \
        {                                                                                                \
            uint32_t   memCpyIndex;                                                                      \
...
#define MATRIXFROMNUMPY(EXT, TYP, SRCTYPE, NUMPYTYPE)                                                \
    ...
    {                                                                                                \
        ...
        PyArrayObject *cdata = (PyArrayObject *) PyArray_FromAny(                                    \
            o, desct, 1, 0, NPY_ARRAY_C_CONTIGUOUS | NPY_ARRAY_ALIGNED | NPY_ARRAY_FORCECAST, NULL); \
        if (cdata)                                                                                   \
        {                                                                                            \
            uint32_t memCpyIndex;                                                                    \
...
static struct PyModuleDef moduledef = {
    PyModuleDef_HEAD_INIT, MODNAME,       NULL, sizeof(struct module_state), CMSISMLMethods, NULL,
    cmsisml_traverse,      cmsisml_clear, NULL};

#define INITERROR return NULL

PyMODINIT_FUNC CAT(PyInit_, MODINITNAME)(void)
